...
function filter_solentvideojw_callback($link) {
   ...
   // echo '<strong>This is the link array</strong><br>';////Test the link array
	...
	// '<br> <strong>This is the streaming folder:</strong> '.$streamingfolder.'<br>';////Test the variable for the streameinf folder
	}
	...
	if ($streamingresource == 'No'){
		...
		if (($filetype != 'flv') && (strpos($_SERVER['HTTP_USER_AGENT'],"iPhone") !==false || strpos($_SERVER['HTTP_USER_AGENT'],"iPad") !==false|| strpos($_SERVER['HTTP_USER_AGENT'],"iPod") !==false || strpos($_SERVER['HTTP_USER_AGENT'],"Android") !==false )){////ITS AN MP4 SO PLAY IT ON IPAD WITH VIDEO TAG
					...
					$output = '<br/> <video controls = "true" '.$size.' loop src="'.$mp4_url.'" ></video><br/><br/>';
	   ...
	   $output = '
	   				  <style>
				  #'.$id.'_wrapper{background-image:url('.$CFG->wwwroot.'/filter/solentvideo/pics/previewsq.jpg); background-repeat: no-repeat;background-position:center; background-color: black; width:'.$width.'; height: '.$height.';}
				  </style>
	...
	if ($streamingresource == 'Yes'){
		...
		if ($streamer == 'estreamp'){/////It is coming from the estreamp server with no need to protect copyright and download
			//echo '<br><strong>This is being delivered from the estreamp server</strong><br>';////Test it is from the estreamp server
			 //echo $_SERVER['HTTP_USER_AGENT'].'<br>';
				if (($filetype != 'flv') && (strpos($_SERVER['HTTP_USER_AGENT'],"iPhone") !==false || strpos($_SERVER['HTTP_USER_AGENT'],"iPad") !==false|| strpos($_SERVER['HTTP_USER_AGENT'],"iPod") !==false || strpos($_SERVER['HTTP_USER_AGENT'],"Android") !==false)){////ITS AN MP4

							
					$mp4_url = $link[1];
					$mp4_url = str_replace('rtmp://estreamp.solent.ac.uk','http://estreamp.solent.ac.uk',$mp4_url);
					$height = '256';
                    ...
                    $size = 'width="'.$width.'" height="'.$height.'"';
					$output = '<br/> <video controls = "true" '.$size.' preload="metadata" loop src="'.$mp4_url.'" poster ="'.$poster.'" ></video><br/><br/>';
				}elseif (($filetype == 'flv') && (strpos($_SERVER['HTTP_USER_AGENT'],"iPhone") !==false || strpos($_SERVER['HTTP_USER_AGENT'],"iPad") !==false|| strpos($_SERVER['HTTP_USER_AGENT'],"iPod") !==false || strpos($_SERVER['HTTP_USER_AGENT'],"Android") !==false)){////ITS AN FLV AND CANT BE SHOWN ON AN IPAD SO SJOW RESOURCE UNAVAILABLE IMAGE
		   ...
		   { type: "flash", src: "'.$CFG->wwwroot.'/filter/solentvideo/player_new.swf" }
				...
				}////END ESTREAMP ELSE CODE
		}////END ESTREAMP CODE
		
		
// VLEMEDIA CODE

		if ($streamer == 'vlemedia'){/////It is coming from the estreamp server with no need to protect copyright and download
			//echo '<br><strong>This is being delivered from the estreamp server</strong><br>';////Test it is from the estreamp server
			// echo $_SERVER['HTTP_USER_AGENT'].'<br>';
...
if (strpos($_SERVER['HTTP_USER_AGENT'],"iPad") !==false){
	echo 'IT IS AN IPAD!!!!<BR>';
					...
					$mp4_url = $link[1];
					if (strpos($_SERVER['HTTP_USER_AGENT'],"Android") !==false){
					$mp4_url = str_replace('rtmp://'.$streamer.'.solent.ac.uk/','http://'.$streamer.'.solent.ac.uk:8134/hds-',$mp4_url.'.f4m');	
					echo $mp4_url.' on Android<br>';				
					}else{
					$mp4_url = str_replace('rtmp://'.$streamer.'.solent.ac.uk/','http://'.$streamer.'.solent.ac.uk/hls-',$mp4_url.'.m3u8');
					echo $mp4_url.' on iPad<br>';
					}
					$height = '256';
					if (strpos(current_theme(),"mobile") !==false){
					 $width ='300px' ;
                    ...
                    $size = 'width="'.$width.'" height="'.$height.'"';
					//echo 'This is the code: <br>
					    // <br/> video controls = "true" '.$size.' autoplay=""  preload="auto" loop src="'.$mp4_url.'" poster ="'.$poster.'" >/video><br/><br/>';
						//$output =  '<video src="'.$mp4_url.'" poster ="'.$poster.'" onclick="this.play();"/>';
					$output =  '<br/> <video controls = "true" '.$size.' preload="metadata" loop src="'.$mp4_url.'" poster ="'.$poster.'" ></video><br/><br/>';
